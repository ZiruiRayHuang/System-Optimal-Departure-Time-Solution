...
nodes_mapping.to_csv(pardir + r'\Preprocessed\nodes_mapping.csv')
print('Done')
print()
...
vehicle_data = [[int(data[index].split()[0]),
                 ...
                 int(float(data[index].split()[3]) / 30)] for index in tqdm(range(2, len(data),2),desc='<1/4>')]
...
vehicle_data = vehicle_data.loc[(vehicle_data['Stime_30min'] >= int(start_time / 30)) & (vehicle_data['Stime_30min'] < int(end_time / 30))]
...
with open(pardir + r'\Preprocessed\t_0_data.dat','wb') as filehandle:
